...
public:
    ...
    static auto get_instance() -> std::shared_ptr<promise_type_t>;

private:
    promise_type_t() = default;

    static std::shared_ptr<promise_type_t> s_instance;

    auto throw_unsupported_operation(const std::string& operation) const -> void;
    auto format_promise_string(std::shared_ptr<object_t> self) const -> std::string;
    auto check_promise_state(std::shared_ptr<object_t> self) const -> void;
};
