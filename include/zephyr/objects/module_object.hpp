...
public:
    ...
    explicit module_object_t(std::shared_ptr<module_t> module, const std::string& module_name, const std::vector<std::string>& allowed_symbols);
...
private:
    ...
    std::vector<std::string> m_allowed_symbols;  // Empty means all symbols allowed
    bool m_filter_symbols;
};
